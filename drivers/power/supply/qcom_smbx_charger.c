			     ...
			     const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		return regmap_update_bits(chip->regmap, chip->base + USBIN_CMD_IL, USBIN_SUSPEND_BIT, !!val->intval);;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
...
static const struct smb_init_register smb2_init_seq[] = {
	{ .addr = AICL_RERUN_TIME_CFG, .mask = AICL_RERUN_TIME_MASK, .val = AIC_RERUN_TIME_3_SECS },
	/*
	 * By default configure us as an upstream facing port
	 * FIXME: This will be handled by the type-c driver
	 */
...
MODULE_DESCRIPTION("Qualcomm SMB Charger Driver");
MODULE_LICENSE("GPL");
